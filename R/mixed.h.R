...
mixedOptions <- if (requireNamespace('jmvcore')) R6::R6Class(
    "mixedOptions",
...
mixedResults <- if (requireNamespace('jmvcore')) R6::R6Class(
...
mixedBase <- if (requireNamespace('jmvcore')) R6::R6Class(
    "mixedBase",
    ...
    public = list(
        initialize = function(options, data=NULL, datasetId="", analysisId="", revision=0) {
            super$initialize(
                ...
                version = c(0,1,0),
                options = options,
                results = mixedResults$new(options=options),
                data = data,
                datasetId = datasetId,
...
mixed <- function(
    ...
    if ( ! requireNamespace('jmvcore'))
        stop('mixed requires jmvcore to be installed (restart may be required)')

    options <- mixedOptions$new(
        dep = dep,
        factors = factors,
        covs = covs,
        cluster = cluster,
    ...
    results <- mixedResults$new(
        options = options)

    analysis <- mixedClass$new(
        options = options,
        data = data)
